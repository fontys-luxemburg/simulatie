...
public class OSMReader : MonoBehaviour {
	...
	void Start () 
	{
        ...
        //Get ways
        XmlNodeList waysXMLList = doc.GetElementsByTagName("way");
        foreach (XmlNode node in waysXMLList)
        {
            long ID = long.Parse(node.Attributes["id"].InnerText);
            ways.Add(new Way(ID));
            XmlNodeList nodes = doc.GetElementById(ID.ToString()).ChildNodes;

            //Voeg node reference aan weg toe.
            int count = 0;
